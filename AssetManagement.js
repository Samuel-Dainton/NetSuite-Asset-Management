...
define(['N/record', 'N/search', 'N/log', 'N/task'], function (record, search, log, task) {
    ...
    function handleLandedCost(itemReceiptId) {
        ...
        log.debug('Total of Items', totalOfItems);
        ...
        for (var i = 0; i < itemCount; i++) {
            ...
            if (inventoryDetailSubrecord) {
                ...
                for (var j = 0; j < inventoryAssignmentCount; j++) {
                    ...
                    log.debug('Serial Number to Push', serialNumber);
            ...
            if (serialNumbers.length > 0) {
                ...
                var assetResults = assetSearch.run().getRange({ start: 0, end: 100 });
                log.debug('Asset Results', assetResults);
    ...
    function getExchangeRate(subsidiaryId, itemReceipt) {
        ...
        log.debug({ title: 'Exchange Rate Date', details: dateToUse });
    ...
    function handleTransferOrder(itemReceipt) {
        ...
        for (var i = 0; i < itemCount; i++) {
            ...
            if (!assetMapping[assetAccount]) {
                log.debug('Item not in asset mapping', 'Item ID: ' + item + ', Asset Account: ' + assetAccount);
